...
interface Modal2RegistrarPagoAlquilerProps {
  open: boolean
...
export default function Modal2RegistrarPagoAlquiler({
  open,
  onOpenChange,
  alquiler,
  onPagoRegistrado
}: Modal2RegistrarPagoAlquilerProps) {
  ...
  useEffect(() => {
    if (open && alquiler?.contratoId) {
      fetchPagosPendientes()
    }
  }, [open, alquiler?.contratoId])

  const fetchPagosPendientes = async () => {
    setLoading(true)
    try {
      const data = await fetchWithToken(`${BACKEND_URL}/alquileres/contrato/${alquiler.contratoId}/pendientes`)
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-lg flex items-center gap-2">
              ...
              Alquiler de {alquiler.direccionInmueble}
            </DialogTitle>
          </DialogHeader>
